...
const juce::String SourceManager::getFilePath(uint64_t ref, SourceType type) const {
	juce::ScopedReadLock locker(audioLock::getSourceLock());

	if (auto ptr = this->getSource(ref, type)) {
		return ptr->getFilePath();
	}
	return "";
}
...
void SourceManager::setAudio(uint64_t ref, double sampleRate, const juce::AudioSampleBuffer& data, const juce::String& path) {
	juce::ScopedWriteLock locker(audioLock::getSourceLock());

	if (auto ptr = this->getSource(ref, SourceType::Audio)) {
		ptr->setAudio(sampleRate, data, path);
	}
}

void SourceManager::setMIDI(uint64_t ref, const juce::MidiFile& data, const juce::String& path) {
	juce::ScopedWriteLock locker(audioLock::getSourceLock());

	if (auto ptr = this->getSource(ref, SourceType::MIDI)) {
		ptr->setMIDI(data, path);
	}
}

void SourceManager::setAudio(uint64_t ref, const juce::String& path) {
	juce::ScopedWriteLock locker(audioLock::getSourceLock());
...
void SourceManager::setMIDI(uint64_t ref, const juce::String& path) {
	juce::ScopedWriteLock locker(audioLock::getSourceLock());
