...
bool AudioCore::save(const juce::String& path) {
	/** Get Save Dir */
	juce::File projFile = utils::getProjectDir().getChildFile(path);
	...
	/** Save Source File */
	this->saveSource(proj.get(), config.projectDir);

	/** Write Project File */
	if (!utils::writeBlockToFile(projFile.getFullPathName(), projData, config.projectDir)) {
...
bool AudioCore::load(const juce::String& path) {
	...
	if (this->parse(proj.get(), config)) {
		/** Load Source File */
		this->loadSource(proj.get(), config.projectDir);
...
void AudioCore::saveSource(const google::protobuf::Message* data,
	const juce::String& baseDir) const {
						...
						if (!savedSet.contains(path)) {
							...
							if (!SourceManager::getInstance()->isSaved(
								...
								SourceIO::getInstance()->addTask(
									{ SourceIO::TaskType::Write, ref, fullPath, false, {} });
							}
						}
					}
				}
...
void AudioCore::loadSource(const google::protobuf::Message* data,
	const juce::String& baseDir) const {
	auto ptrProj = dynamic_cast<const vsp4::Project*>(data);
	if (!ptrProj) { return; }
	...
	for (int i = 0; i < mainGraph->getTrackNum(MainGraph::TrackType::Track); i++) {
		if (auto track = mainGraph->getTrackProcessor(MainGraph::TrackType::Track, i)) {
			if (auto seq = track->getSequencer()) {
				...
				if (!seqData.midisrc().empty()) {
					if (auto ref = seq->getMIDIRef()) {
						juce::String path = juce::File{ baseDir }
