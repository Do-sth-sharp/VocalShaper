...
const juce::String SeqSourceProcessor::getAudioFilePath() const {
	juce::String path = this->getAudioPath();
	if (path.isEmpty()) { path = utils::getLegalFileName(this->getNameFunc()); }
	if (path.isEmpty()) { path = juce::String{ this->index }; }
	...
	auto newName = file.getFileNameWithoutExtension() + extension;
	return juce::File::isAbsolutePath(path) ?
		file.getSiblingFile(newName).getFullPathName() : newName;
}

const juce::String SeqSourceProcessor::getMIDIFilePath() const {
	juce::String path = this->getMIDIPath();
	if (path.isEmpty()) { path = utils::getLegalFileName(this->getNameFunc()); }
	if (path.isEmpty()) { path = juce::String{ this->index }; }
...
const juce::String SeqSourceProcessor::getAudioPath() const {
	return SourceManager::getInstance()->getFilePath(
		this->audioSourceRef, SourceManager::SourceType::Audio);
}

const juce::String SeqSourceProcessor::getMIDIPath() const {
	return SourceManager::getInstance()->getFilePath(
